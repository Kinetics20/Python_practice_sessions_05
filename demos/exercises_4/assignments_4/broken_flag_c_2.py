...
class PNGRepairer:
    ...
    def get_pow_token(self):
        ...
        try:
            ...
            while True:
                ...
                if hash_val.startswith('ffffff'):  # Top 24 bits set to 1
                    pow_token = candidate
                    break
                nonce += 1
                ...
                if nonce % 10000 == 0:
                    ...
                    print(f"Checked {nonce} combinations in {elapsed:.1f}s... Current hash: {hash_val[:8]}...")

                # Safety timeout after 5 minutes
                if time.time() - start_time > 300:
                    print("PoW taking too long, something might be wrong!")
                    return None

            elapsed = time.time() - start_time
            print(f"Found PoW: {pow_token} (took {elapsed:.1f}s)")
            with self.lock:
    ...
    def get_hash_from_server(self, offset, size):
        ...
        try:
            ...
            print(f"Requesting hash for offset={offset}, size={size}")
            response = self.session.get(url, params=params, timeout=40)
            response.raise_for_status()

            hash_result = response.json()['hash']
            print(f"Got hash: {hash_result}")
            return hash_result
    ...
    def get_data_from_server(self, offset):
        ...
        try:
            ...
            print(f"Requesting data for offset={offset}")
            response = self.session.get(url, params=params, timeout=40)
            response.raise_for_status()
    ...
    def find_corrupted_blocks_binary_search(self, start_offset, end_offset, depth=0):
        """Find corrupted blocks using binary search"""
        indent = "  " * depth
        print(f"{indent}Checking range {start_offset}-{end_offset}")

        if end_offset - start_offset <= 32:
            if start_offset < self.file_size:
                print(f"{indent}Found leaf block at {start_offset}")
                return [start_offset]
            else:
                return []
        ...
        if not server_hash:
            print(f"{indent}Failed to get server hash")
            return []
        ...
        if server_hash == our_hash:
            print(f"{indent}✓ Block {start_offset}-{start_offset + size} is correct")
            return []

        print(f"{indent}✗ Block {start_offset}-{start_offset + size} is corrupted, splitting...")
        ...
        corrupted = []
        corrupted.extend(self.find_corrupted_blocks_binary_search(start_offset, mid_offset, depth + 1))
        corrupted.extend(self.find_corrupted_blocks_binary_search(mid_offset, end_offset, depth + 1))
    ...
    def find_all_corrupted_blocks(self):
        ...
        print(f"Aligned file size: {aligned_size} bytes")
...
def main():
    print("Starting PNG Repairer with debug logging...")
    repairer = PNGRepairer('brokenflag.png', max_workers=4)

    # Step 1: Find corrupted blocks
    print("\n=== STEP 1: Finding corrupted blocks ===")
    ...
    print("\n=== STEP 2: Repairing corrupted blocks ===")
